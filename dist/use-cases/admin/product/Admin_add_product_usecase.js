...
class Admin_add_product_Usecase {
    constructor(productRepositories, categoryRepositories) {
        this.productRepositories = productRepositories;
        this.categoryRepositories = categoryRepositories;
    }
    execute(name, description, stock, price, categoryName, images) {
        return __awaiter(this, void 0, void 0, function* () {
            const existProduct = yield this.productRepositories.productFindbyName(name);
            if (existProduct)
                throw new Error("product already exist");
            const categoryId = yield this.categoryRepositories.categoryFindbyName(categoryName);
            if (!categoryId)
                throw new Error("category not exist..");
            const productDate = new ProductEntities_1.Product_Entities("", name, description, stock, categoryId, price, images);
            const product = yield this.productRepositories.productCreate(productDate);
