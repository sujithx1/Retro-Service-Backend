  ...
  async execute(
    ...
    if (!storeWallet)
      throw new CustomError(
        "storeWallet not Found",
        404,
        AppError.ResourceNotFound
      );

    storeWallet.balance = total;
    const updatestoreWallet =
      await this.walletrepositories.findByuserIdandUpdate(storeWallet);

    if (!updatestoreWallet)
      throw new CustomError(
        "storeWallet not updated",
        500,
    ...
    const usertransaction = new TransactionEntities(
      ...
      Number(total),
      "complete",
      paymentMethod.trim() as "razorpay" | "wallet" | "cod",
      "product"
    );

    await this.transactionrepositories.create(usertransaction);
    if (paymentMethod == "wallet") {
      const userwallet = await this.walletrepositories.findByuserId(
        cart.userId.toString()
      );
      if (userwallet) {
